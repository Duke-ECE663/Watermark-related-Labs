...
def main():
    ...
    for train in ['standard', 'adversarial']:
        checkpoint = torch.load('./checkpoint/'+train+'.pth')
        ...
        print(train+' training FPR:', np.sum(certified_fp >= args.tau, axis=1) / num_data)
    ...
    for train in ['standard', 'adversarial']:
        ...
        print(train+' training FNR:', np.sum((certified_fn < args.tau), axis=1) / num_data)
